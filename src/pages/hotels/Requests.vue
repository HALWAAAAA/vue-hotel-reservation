<template>
  <div class="max-w-3xl mx-auto py-8">
    <h1 class="text-3xl font-extrabold mb-8 text-gray-800">My Booking Requests</h1>
    <ul v-if="requests.length" class="grid gap-6">
     ...
     >
        <div>
          <h2 class="text-lg font-semibold text-gray-900">{{ r.hotelName || '—' }}</h2>
          <p class="mt-1 text-gray-600">Room <span class="font-medium">{{ r.roomId }}</span></p>
          <p class="mt-2 text-sm text-gray-500">{{ r.startDate }} &mdash; {{ r.endDate }}</p>
        </div>
        <Button
